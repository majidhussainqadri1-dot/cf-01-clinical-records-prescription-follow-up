...
| CF01-A-009 | Freeze File 00/02 identity, guardian, suspension, recent-auth and capability assertion contract | File 00/02 Owners | File 00 `1.2.7` provider Draft PR #13 and File 02 `0.3.0` Draft PR #4 are implemented, twice reviewed and exact-head CI-green; File 02 now includes both `sa.cf01.authentication-assurance` and session-bound `sa.professional-reauthentication`; evidence recorded in `C1-A-CROSS-REPOSITORY-CONTRACT-TRACKING.md` | merge-order resolution, immutable merged-version provider/consumer tests, native-owner acceptance, staging/provider sandbox, privacy/security review and Founder change control | In Review; acceptance blocked |
| CF01-A-010 | Freeze File 08 appointment/care-context extraction boundary | File 08 Owner / Clinical Product Owner | File 08 `0.2.2` scheduling-only provider Draft PR #5, field/write-path extraction inventory and deterministic candidate are implemented, twice reviewed and exact-head CI-green; appointment/clinical authority separation and zero narrative leakage are tested | File 08 base PR #3 resolution, immutable File 00/File 09 dependencies, merged-version CF-01 consumer tests, current staging-data inventory, extraction dry run/reconciliation/rollback, native-owner and privacy/security/clinical review, Hostinger staging and Founder change control | In Review; acceptance blocked |
| CF01-A-011 | Freeze doctor professional-verification contract with Files 03/07/09 | Relevant Module Owners | File 09 `1.1.1` `gdo.cf01.practitioner-eligibility` Draft PR #4 and File 02 professional-reauthentication bridge are implemented, twice reviewed and exact-head CI-green; current membership/contact/2FA/guardian truth, opaque subject binding, professional row version, snapshot fingerprint, evidence validity, jurisdiction and non-authorization limits are tested | File 09 base PR #2 and File 00/02 dependency resolution, Files 03/07 field reconciliation, structured professional-restriction ownership, immutable merged-version producer/CF-01 consumer tests, native-owner acceptance, legal/professional/privacy/security review, staging/rollback and Founder change control | In Review; acceptance blocked |
...
| CF01-A-022 | Enforce presence and semantic minimums of the public C1-A governance package in CI | QA / Security | repository validator and tests; File 00/File 02/File 08/File 09 evidence reconciled into central tracking | green CI on the current central head and every subsequent change | In Review |
